...
* [`src/vtakt_client/styles.cljs`](src/vtakt_client/styles.cljs): CSS compilation source file (ClojureScript,
[Garden](https://github.com/noprompt/garden))
* [`src/vtakt_client/`](src/vtakt_client/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/vtakt_client/core.cljs): contains the SPA entry point, `init`
* [`test/vtakt_client/`](test/vtakt_client/): test files (ClojureScript,
...
(headless test environment)
    * For Chromium, set the `CHROME_BIN` environment variable in your shell to the command that
    launches Chromium. For example, in Ubuntu, add the following line to your `.bashrc`:
       ...
       ```
5. Clone this repo and open a terminal in the `vtakt-client` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/vtakt_client/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns vtakt-client.example
  (:require [vtakt-client.config :as config])
